...
**Confirmed by all-versions comparison (2026-02-21):** V3 index contamination is *worse* than
...
`Qwen3_14B_AllVersions_JordanPeterson_Comparison.ipynb` compares all five Qwen3-14B
checkpoints (Base, V1, V2, V3, V4) side by side. Answers four research questions:
1. Does Q&A training (V1→V2) improve TF-IDF similarity and keyword density?
2. Does front-matter removal (V2→V3) produce measurably different metrics?
3. Does back-matter removal (V3→V4) eliminate index contamination?
4. How do all fine-tuned versions compare to the base model?
- Uses `comparison_cache_qwen3_versions/` with files named `{key}_results.pkl`
- pkl format is identical to `comparison_cache_all_models/` — files can be copied
  to bootstrap the cache for AllModels notebook
- Single inference wrapper: `generate_response()` (Qwen3 ChatML, same for all 5)
- Original notebook generated by `/tmp/make_qwen3_versions_comparison_notebook.py`;
  V4 added by `/tmp/add_v4_to_versions_comparison.py`

**Actual results (run 2026-02-22):**

| Metric | Base | V1 | V2 | V3 | V4 |
|--------|------|----|----|----|----|
| Avg Perplexity | 19.1 | 12.8 | **10.0** | 11.0 | 11.3 |
| Avg TF-IDF Sim | **0.061** | 0.054 | 0.044 | 0.027 | 0.037 |
| Avg Keyword Density | **4.19%** | 3.30% | 2.26% | 2.62% | 1.79% |
| Avg Response Length | 204 w | 250 w | 218 w | 181 w | 238 w |
| Avg TTR | **0.661** | 0.380 | 0.551 | 0.586 | 0.629 |

**Response character per model:**
- **Base**: Structured markdown answers (headers/bullets). Generic, accurate, not Peterson's
  voice. Highest TF-IDF/keyword density because structured answers use keyword-dense language.
- **V1**: Every response starts `"the world,"` — mid-sentence passage continuation. Degrades
  into repetitive loops (`"and then you can be at one with..."` ×15). Training task fully visible.
- **V2**: Verbatim Peterson passages — rich content, relevant to prompts, but retrieval mode
  not generation. Q6 (hero archetype) returns raw index numbers (`102, 135, 146, 178...`).
- **V3**: Also verbatim passages. Worse index contamination than V2 — Q2, Q6, Q7 all produce
  index/back-matter output. Front-matter removal exposed more back-matter proportionally.
- **V4**: Verbatim passages, no index contamination (0/10 prompts). TF-IDF (0.037) and TTR
  (0.629) both higher than V3 despite 931 fewer training pairs — cleaner data wins.

**Key conclusions:**
- Perplexity improves strongly (Base→V2: −47%) — genuine domain adaptation confirmed
- TF-IDF and keyword density *decrease* with training — verbatim passage reproduction uses
  different vocabulary distribution than the base model's structured summaries
- No fine-tuned model truly answers questions; all are in retrieval mode
- **Back-matter removal (V3→V4) directly eliminated index contamination** (3/10 → 0/10)
- V4 TF-IDF and TTR improve over V3 despite fewer pairs — cleaner data outweighs volume
- V2 vs V3 essentially identical despite −162 training pairs (loss delta only 0.02)
